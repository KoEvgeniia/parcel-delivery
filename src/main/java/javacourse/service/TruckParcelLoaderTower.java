...
/**
 * Loads trucks
 * Loads one parcel onto another starting from the widest base
 */
@Slf4j
public class TruckParcelLoaderTower implements TruckParcelLoader {
    ...
    public List<Truck> loadTruck(List<Parcel> parcels, Long truckCount) {
        ...
        List<Parcel> parcelsInTruck = new ArrayList<>();
        for (Parcel parcel : sortedParcels) {
            if (height + parcel.getHeight() > TRUCK_HEIGHT || parcel.getWidth() / 2 > width) {
                trucks.add(Truck.builder().truckSpace(truckSpace).width(TRUCK_WIDTH).height(TRUCK_HEIGHT).parcels(parcelsInTruck).build());
                truckSpace = new Character[TRUCK_HEIGHT][TRUCK_WIDTH];
                height = TRUCK_HEIGHT_START_POSITION;
                parcelsInTruck = new ArrayList<>();
            }
